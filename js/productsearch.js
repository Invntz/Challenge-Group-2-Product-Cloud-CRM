...
function displayData(data) {
    ...
    data.forEach(product => {
        ...
        row.innerHTML = `
           ...
           <td id="tagtable">
           ${(Array.isArray(product.TagName) && product.Size.length > 1) 
            ? `<ul>${product.TagName.map(size => `<li id="tagLi">${size}</li>`).join('')}</ul>` 
            : Array.isArray(product.TagName) 
            ?  `<li id="tagLi">${product.TagName.join(', ')}</li>`
            :  `<li id="tagLi">${product.TagName}</li>`}
           </td>`;
